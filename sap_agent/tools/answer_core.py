"""Shared answer-tool core (split from answer.py): snapshot, coerce, freeze.
...
from ..schemas import AnsweredQuestion, IntentConfig, QuestionIntent
...
def _wait_for_table_rows(page: PageLike, timeout_ms: int = 2_000) -> None:
...
def _snapshot(page: PageLike, ctx: SessionContext) -> _TableSnapshot:
...
def fetch_json_body(capture: CaptureLike | None, stem: str) -> list[dict] | None:
